                ...
                '<div id="{[this.id]}-wk-{[xindex-1]}" class="ext-cal-wk-ct" style="top:{[this.getRowTop(xindex, xcount)]}%; height:{[this.getRowHeight(xcount)]}%;">',
                    ...
                    '<table class="ext-cal-evt-tbl" cellpadding="0" cellspacing="0">',
                        '<tbody>',
                            '<tr>',
                                '<tpl for=".">',
                                    '<td id="{[this.id]}-ev-day-{date:date("Ymd")}" class="{titleCls}"><div style="{hDayCls}">{title}</div></td>',
                                '</tpl>',
                            '</tr>',
                        '</tbody>',
                    '</table>',
    ...
    applyTemplate : function(o){
        ...
        for(; w < this.weekCount || this.weekCount == -1; w++){
            ...
            for(var d = 0; d < this.dayCount; d++){
                ...
                var record = null, isHoliday, lunarDate;
                
                if( this.holidayStore ) {
                	record = this.holidayStore.getById(Ext.Date.format(dt, 'Ymd'));
                	// record = this.holidayStore.findRecord('date', Ext.Date.format(dt, 'Ymd'));
            	} 
                isHoliday = ((record && record.get('isHoliday'))||(Ext.Date.format(dt, 'D')==='Sun'));
                lunarDate = (record && record.get('lunarDate')) ? "[" + record.get('lunarDate') +"]&#160;" : "";
                	
                weeks[w].push({
                    title: ( (record&&record.get('title')) ? record.get('title')+'&#160;' : '') + lunarDate + title,
                    // hDayCls : ((record && record.get('isHoliday'))||(Ext.Date.format(dt, 'D')==='Sun')) ? 'color:red' : '',
                    date: Ext.Date.clone(dt),
                    titleCls: 'ext-cal-dtitle ' + (isToday ? ' ext-cal-dtitle-today' : '') +
                        (w==0 ? ' ext-cal-dtitle-first' : '') +
                        (prevMonth ? (' ext-cal-dtitle-prev' + (isHoliday ? '-holiday' : (Ext.Date.format(dt, 'D')==='Sat' ? '-sat':''))) : '') +
                        (nextMonth ? (' ext-cal-dtitle-next' + (isHoliday ? '-holiday' : (Ext.Date.format(dt, 'D')==='Sat' ? '-sat':''))) : '')  +
                        ( (isHoliday && !prevMonth && !nextMonth) ? ' ext-cal-dtitle-holiday' : (Ext.Date.format(dt, 'D')==='Sat' ? ' ext-cal-dtitle-sat':'')) ,
