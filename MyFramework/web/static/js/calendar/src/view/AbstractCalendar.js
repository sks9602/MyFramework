...
Ext.define('Ext.calendar.view.AbstractCalendar', {
    ...
    prepareEventGrid: function(evts, w, d) {
        ...
        evts.each(function(evt) {
            ...
            if (days > 1 || Ext.calendar.util.Date.diffDays(evt.data[M.StartDate.name], Ext.calendar.util.Date.add(evt.data[M.EndDate.name], { days : (evt.data[M.IsAllDay.name] ? 1 : 0)})) > 1) {
    ...
    isEventVisible: function(evt) {
    	var M = Ext.calendar.data.EventMappings,
            ...
            evEnd = Ext.calendar.util.Date.add(data[M.EndDate.name], { days : (data[M.IsAllDay.name] ? 1 : 0) , seconds: -1}).getTime();
