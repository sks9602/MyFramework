...
Ext.define('Ext.calendar.view.Month', {
    ...
    getTemplateEventData: function(evt) {
        ...
        /*
        // 시작일자..
        if( Ext.calendar.util.Date.diffDays(evt[M.StartDate.name], Ext.calendar.util.Date.add(evt[M.EndDate.name], { days : (evt[M.IsAllDay.name] ? 1 : 0)})) > 1 ){
        	dt += Ext.Date.format(evt[M.StartDate.name], 'Y.m.d ') ;
        }
        // 시작시간..
        if( !evt[M.IsAllDay.name] ) {
        	dt += Ext.Date.format(evt[M.StartDate.name], ' g:ia '); 
        }
        ...
        */
        ...
        return Ext.applyIf({
            ...
            // Title: dt + (!title || title.length == 0 ? '(No title)' : title)
            Title: evt.TitlePrefix + (!title || title.length == 0 ? '(No title)' : title) + evt.TitleSuffix
        },
        evt);
    },
    ...
    getMaxEventsPerDay: function() {
        var dayHeight = this.getDaySize(false).height,
