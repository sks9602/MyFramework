		...
		<jun:grid id="grid_2" title="권한 대상자 현황" action="/system/auth.do" page="1" button="추가,저장,삭제" lead="checkbox"  fields="Auth.selectAuthMemberList" position="sub">
			<jun:objects name="window">
					popupMember : Ext.widget('window', {
				        ...
				        items : [
							<jun:data>
								<jun:form section="search"  button="조회" gridId="id_member" position="north" maxItem="4" id="search_member">
									<jun:itemText  type="hidden"  name="p_action_flag" value="r_auth_list" ></jun:itemText>
									<jun:itemText  type="text"   name="AUTH_NM"  title="권한 구분"        ></jun:itemText>
								</jun:form>
								<jun:grid action="/system/auth.do" id="id_member" page="3" width="500" fields="Auth.selectAuthList">
									<jun:columns>
										<jun:column title="성명" column="AUTH_ID" width="120"></jun:column>
										<jun:column title="사번" column="AUTH_NM" flex="1" >
											renderer : function (value, metaData, record){
												
												return Ext.String.format(
													"<span class=\"in_grid_url_link\">{0}</span>",
												value,
												record.data.AUTH_ID,
												metaData );
											},
