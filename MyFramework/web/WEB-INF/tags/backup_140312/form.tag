	...
	if( buttons.length> 0 ) {
		...
		for( int i=0 ; i< buttons.length ; i++ ) {
			...
			if( "조회".equals(buttons[i])) {
				out.println("{ xtype : 'button', text: '"+buttons[i]+"', handler : function () { if( this.up('form').getForm().isValid() ) { Ext.getStore('store_grid_"+param.get("p_action_flag")+"_"+HoUtil.replaceNull(gridId) +"').load({params: this.up('form').getForm().getFieldValues(false)}); } else { this.up('form').getForm().findInvalid(); } } }");
			} else {
