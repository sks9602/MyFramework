...
import org.apache.ibatis.metadata.result.MetaDataRow;
...
public class FastResultSetHandler implements ResultSetHandler {
  ...
  public List handleResultSets(Statement stmt) throws SQLException {
	...
	MetaData metaData = setMetaData(rsmd);
    //@ sks end
	...
	list.add(metaData);
	list.addAll(collapseSingleResultList(multipleResults));
	return list;
    //@ sks end
  }
  ...
  protected Object getDiscriminatorValue(ResultSet rs, Discriminator discriminator) throws SQLException {
	...
	private MetaData setMetaData(ResultSetMetaData rsmd) {
		MetaData metaData = new MetaData();
		MetaDataRow row = null;
		try {
			metaData.setColumnCount(rsmd.getColumnCount());
			for (int i = 0; i < rsmd.getColumnCount(); ++i) {
				row = new MetaDataRow();
				row.setAutoIncrement(rsmd.isAutoIncrement(i + 1));
				row.setCaseSensitive(rsmd.isCaseSensitive(i + 1));
				row.setCatalogName(rsmd.getCatalogName(i + 1));
				row.setColumnClassName(rsmd.getColumnClassName(i + 1));
				row.setColumnDisplaySize(rsmd.getColumnDisplaySize(i + 1));
				row.setColumnLabel(rsmd.getColumnLabel(i + 1));
				row.setColumnName(rsmd.getColumnName(i + 1));
				row.setColumnType(rsmd.getColumnType(i + 1));
				row.setColumnTypeName(rsmd.getColumnTypeName(i + 1));
				row.setCurrency(rsmd.isCurrency(i + 1));
				row.setDefinitelyWritable(rsmd.isDefinitelyWritable(i + 1));
				row.setNullable(rsmd.isNullable(i + 1));
				row.setPrecision(rsmd.getPrecision(i + 1));
				row.setReadOnly(rsmd.isReadOnly(i + 1));
				row.setScale(rsmd.getScale(i + 1));
				row.setSchemaName(rsmd.getSchemaName(i + 1));
				row.setSearchable(rsmd.isSearchable(i + 1));
				row.setSigned(rsmd.isSigned(i + 1));
				row.setTableName(rsmd.getTableName(i + 1));
				row.setWritable(rsmd.isWritable(i + 1));

				metaData.setMetaDataRow(row);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
