...
public class HoPostLoggingAdvice implements AfterReturningAdvice {
	...
	public void afterReturning(Object returnValue, Method method, Object[] args, Object target) throws Throwable {
		...
		if( returnValue instanceof List) {
			...
			for( int i=0 ; i<((String[])returnValue).length && i < 50 ; i++ ) {
			...
			logger.debug( "Return : "+ sb );
		} else if( returnValue != null){
			logger.debug("Return : "+returnValue.getClass().getName() + "-->"+ returnValue.toString());
		} else {
			logger.debug("Return : --> [null]");
		}
		logger.debug("Ending : "+className+"."+method.getName()+"()");
	}
